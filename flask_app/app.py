...
def process_stock_data(symbol, ndays, window, std_dev, timeframe, num_samples):
    TIMEFRAME = TimeFrame.Minute if timeframe == "Minute" else TimeFrame.Day
    END_DATE = date.today()
    NYSE = mcal.get_calendar("NYSE")
    START_DATE = NYSE.valid_days(start_date=END_DATE - timedelta(days=2 * ndays), end_date=END_DATE)[-ndays]
    SYMBOL = symbol.upper()
    DATA_CONFIG = DataConfig(SYMBOL, TIMEFRAME, START_DATE, END_DATE, ndays, timeframe, num_samples)
...
def analyze_stock():
    ...
    ndays = int(request.form.get('ndays', 504))
    window = max(int(request.form.get('window', 0)), 0)
    std_dev = float(request.form.get('std_dev', 0.01))
